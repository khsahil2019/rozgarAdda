...
class ExploreCareerScreen extends StatefulWidget {
  const ExploreCareerScreen({super.key});

  @override
  State<ExploreCareerScreen> createState() => _ExploreCareerScreenState();
}

class _ExploreCareerScreenState extends State<ExploreCareerScreen> {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Obx(() {
          ...
          final selectedCategoryName = selectedCategory?.name ?? 'Job Category';
          final selectedScopeName =
              selectedSubCategory?.name ?? selectedCategoryName;
          ...
          return Column(
            children: [
              _buildHeader(selectedScopeName),
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      _buildFilterRow(categories, subCategories),
                      const SizedBox(height: 18),
                      _buildLocationRow(selectedScopeName),
                      ...
                      const SizedBox(height: 18),
                      if (_controller.isLoading.value && categories.isEmpty)
                        const Padding(
                          ...
                          categories.isEmpty)
                        _buildErrorState()
                      else if (_controller.isSubCategoriesLoading.value ||
                          _controller.isJobRolesLoading.value)
                        const Padding(
                          padding: EdgeInsets.symmetric(vertical: 36),
                          child: Center(
                            child: CircularProgressIndicator(
                      ...
                      else if (_controller.subCategoriesError.value != null)
                        _buildMessageState(
                          _controller.subCategoriesError.value!,
                          onRetry: () {
                            final category = _selectedCategory(categories);
                            if (category != null) {
                              _controller.fetchSubCategories(category.id);
                            }
                          },
                        )
                      else if (_controller.jobRolesError.value != null)
                        _buildMessageState(
                          _controller.jobRolesError.value!,
                          onRetry: () {
                            final subCategory = _selectedSubCategory(
                              subCategories,
                            );
                            if (subCategory != null) {
                              _controller.fetchJobRoles(subCategory.id);
                            }
                          },
                        )
  ...
  DashboardCategory? _selectedCategory(List<DashboardCategory> categories) {
    if (categories.isEmpty) return null;
    final safeIndex = _selectedCategoryIndex.clamp(0, categories.length - 1);
    return categories[safeIndex];
  }

  DashboardSubCategory? _selectedSubCategory(
    List<DashboardSubCategory> subCategories,
  ) {
    if (subCategories.isEmpty) return null;
    final safeIndex = _selectedSubCategoryIndex.clamp(
      0,
      subCategories.length - 1,
    );
    return subCategories[safeIndex];
  }

  List<JobListing> _buildJobs(
    String scopeName,
    ...
    final roles = _controller.jobRoles;
  ...
  Widget _buildFilterRow(
    ...
    final subCategoryLabel = subCategories.isEmpty
        ? 'Sub Category'
        ...
        children: [
          ...
          _buildFilterChip(
            ...
            onTap: subCategories.isEmpty
                ...
                : () => _showFilterSheet(
                    categories: categories,
                    initialSection: _FilterSection.subCategory,
                  ),
          ),
          const SizedBox(width: 10),
          _buildFilterChip(
  ...
  Widget _buildCategoryHint(
    ...
    DashboardSubCategory? selectedSubCategory,
  ) {
    final text = selectedSubCategory == null
        ? 'Select a sub-category under $selectedCategoryName'
        : 'Showing ${selectedSubCategory.name} jobs in $selectedCategoryName';
    return Container(
                                ...
                                Expanded(
                                  child:
                                      ...
                                      ? Obx(() {
                                          ...
                                          final error = _controller
                                              .subCategoriesError
                                              .value;
                                          if (error != null) {
                                            return _FilterInlineState(
                                              message: error,
                                              actionLabel: 'Retry',
                                              onAction: () {
                                                final category =
                                          ...
                                          if (subCategories.isEmpty) {
                                            return const _FilterInlineState(
                                              message:
  ...
  String _sectionTitle(_FilterSection section) {
    return switch (section) {
      ...
      _FilterSection.subCategory => 'Sub Category',
  ...
  List<String> _optionsForSection(
    ...
    return switch (section) {
      ...
      _FilterSection.subCategory => [
        'All Sub Categories',
        ..._controller.subCategories.map((item) => item.name),
      ],
  ...
  int _selectedIndexForSection(
    ...
    return switch (section) {
      ...
      _FilterSection.subCategory =>
        subCategories.isEmpty ? 0 : _selectedSubCategoryIndex + 1,
  ...
  void _setSelectedIndexForSection(
    ...
    switch (section) {
      case _FilterSection.category:
        ...
        if (categories.isNotEmpty) {
          _controller.selectCategory(categories[_selectedCategoryIndex]);
        }
        return;
      case _FilterSection.subCategory:
        _selectedSubCategoryIndex = index == 0 ? 0 : index - 1;
        if (subCategories.isNotEmpty) {
          _controller.selectSubCategory(
...
class _SubCategoryFilterCard extends StatelessWidget {
  final DashboardSubCategory subCategory;
  final bool isSelected;
  final VoidCallback onTap;
...
class _FilterInlineState extends StatelessWidget {
  final String message;
  final String? actionLabel;
  final VoidCallback? onAction;

  const _FilterInlineState({
